...
export async function runStatus({ api, state, probe }: Deps): Promise<string[]> {
  ...
  if (!outputs) return [...lines, '스택 없음 — tkf up으로 생성하세요'];

  const asgName = await api.getAsgName(stackName);
  const { desired, instanceIds } = await api.getAsgStatus(asgName);
  if (instanceIds.length === 0) {
    return [...lines, desired === 0 ? '정지됨 (desired=0) — tkf up으로 기동' : '스팟 확보 대기 중 (desired=1, 인스턴스 0대)'];
  }
  ...
  // 403은 CloudFront 엣지 차단(allowedCidrs 허용목록 밖)이므로 부팅 중과 구분해 표시한다.
  const readyLine = code === 200 || code === 401 ? `READY — ${outputs.EndpointUrl}`
    : code === 403 ? '차단됨 (403 — allowedCidrs 허용목록 확인)'
    : `부팅 중 (엔드포인트 ${code || '연결 불가'})`;
